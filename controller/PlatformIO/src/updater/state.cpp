...
void updateState()
{
    ...
    if (currentState == MOVING)
    {
        ...
        {
            setspeedroue(TURN_SPEED); // Pour LEFT et RIGHT
        }

        if (tempsActuel >= tempsFinAction)
        {
            currentState = IDLE;
            Serial.print("Temps de commande : ");
            Serial.println(tempsFinAction);
    ...
    if (currentState == MOVING && (tempsActuel - dernierMessagePi > timeoutPi))
    {
        currentState = IDLE;
        Serial.println("[ALERTE] Watchdog : Perte de connexion RBI !");
    }
